	...
	public String getPageList(int pageNum, int postNum, int pageListCount, int totalCount, String keyword) {
		...
		if (totalPage != 1) {
			...
			for (int i = 1; i <= pageListCount; i++) {
				...
				// - i == 1
				if (section > 1 && i == 1)
					pageList += " <a href=/board/list?page=" + Integer.toString((section - 2) * pageListCount + pageListCount)
					+ "&keyword=" + keyword + ">◀</a>";
				//2. 페이지 출력 중단
				...
				if (pageNum != (section - 1) * pageListCount + i)
					pageList += " <a href=/board/list?page=" + Integer.toString((section - 1) * pageListCount + i)
				...
				if (i == pageListCount && totalSection > 1 && totalPage >= i + (section - 1) * pageListCount + 1)
					pageList += " <a href=/board/list?page=" + Integer.toString(section * pageListCount + 1) + "&keyword=" + keyword + ">▶</a>";
